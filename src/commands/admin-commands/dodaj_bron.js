...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('dodaj_bron')
		.setDescription('Dodaj postaci broń')
        ...
        .addStringOption(option =>
            option
                .setName('bron')
                .setDescription('broń którą otrzyma postać')
                .setRequired(true)
                .setAutocomplete(true)
        )
	,
    async autocomplete(interaction) {
        ...
        if(focusedOption.name === 'imie'){
            ...
            weapons.forEach((element) => choices.push(element.display_name));
        }
	...
	async execute(interaction) {
        ...
        character[0].weapons.push(weapon[0].name);
        var message = '```ansi\n[1;32m' + character[0].name + '[1;37m otrzymuje ' + weapon[0].display_name + '[0m\n```';
