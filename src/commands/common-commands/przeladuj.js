...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('przeladuj')
		.setDescription('Przeładuj kuszę')
	...
	async autocomplete(interaction) {
        ...
        if(focusedOption.name === 'imie'){
            ...
            character[0].weapons.forEach((weapon_name) =>{
                ...
                if(weapon[0].type === 'crossbow') choices.push(weapon[0].display_name)
            });
	...
	async execute(interaction) {
        ...
        if (eval('character[0].reload.' + weapon[0].name + '< 1')){
            eval('character[0].reload.' + weapon[0].name + ' += 1');
        }
        else {
            var message = '```ansi\n[1;37m' + weapon[0].display_name + '[1;31m jest już przeładowana[0m\n```';
            await interaction.editReply(message);
            return
        }

        var message = '```ansi\n[1;32m' + character[0].name + '[1;37m przeładowuje ' + weapon[0].display_name + '\n```';
